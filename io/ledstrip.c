...
static void applyLedGpsLayer(bool updateNow, timeUs_t *timer)
{
...
#ifdef USE_GPS_RESCUE
    const bool satsSatisfied = (STATE(GPS_FIX) && (gpsSol.numSat >= gpsRescueConfig()->minSats));
#else
    const bool satsSatisfied = STATE(GPS_FIX);
#endif
    ...
    } else if (satsSatisfied) {
        // ✅ 2. 위성수 만족 -> 아밍 전이라도 구조 가능 조건을 충족하면 초록색 고정
        gpsColor = getSC(LED_SCOLOR_GPSLOCKED);
    } else {
        // ✅ 1. 위성수 미달 또는 No Fix -> 주황색/빨간색 깜빡임 (준비 중)
        bool colorOn = gpsPauseCounter == 0;
