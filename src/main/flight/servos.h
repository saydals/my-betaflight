...
typedef enum {
    ...
    INPUT_GIMBAL_PITCH,
    INPUT_GIMBAL_ROLL,
    INPUT_BIRD_FLAP,        // id=14: Bird Flap biomimetic wing control
    INPUT_SOURCE_COUNT
} inputSource_e;
...
// ============================================================
// Bird Flap configuration (PG 59, stored in EEPROM)
// ============================================================
typedef enum {
    BIRD_FLAP_STATE_OFF = 0,
    BIRD_FLAP_STATE_STARTING,
    BIRD_FLAP_STATE_ACTIVE,
    BIRD_FLAP_STATE_STOPPING
} birdFlapState_e;

typedef struct birdFlapConfig_s {
    uint16_t maxFreqX10;        // 0.5~4.0Hz stored as value*10 (range 5~40)
    uint16_t minFreqX10;        // 0.1~1.0Hz stored as value*10 (range 1~10)
    uint16_t servo_speed;       // degrees/second (range 10~500)
    uint16_t up_amplitude;      // upward stroke amplitude in servo units (range 0~500)
    uint16_t down_amplitude;    // downward stroke amplitude in servo units (range 0~500)
    uint16_t upRatio100x;       // 0.30~0.80 stored as value*100 (range 30~80)
    uint16_t softStartMs;       // soft start time in ms (range 100~3000)
    uint16_t softStopMs;        // soft stop time in ms (range 100~3000)
    uint16_t freqTauMs;         // frequency LPF time constant in ms (range 10~1000)
    uint16_t slewRate;          // output slew rate in units/s (range 0~10000, 0=disabled)
} birdFlapConfig_t;

PG_DECLARE(birdFlapConfig_t, birdFlapConfig);
