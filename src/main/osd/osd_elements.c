...
#ifdef USE_GPS
static void osdFormatCoordinate(char *buff, int32_t gpsValue, gpsCoordinateType_e coordinateType, osdElementType_e variantType, bool showLeadingSymbol)
{
...
#define PLUS_CODE_DIGITS 11
    case OSD_ELEMENT_TYPE_4: // Open Location Code
        {
            if (showLeadingSymbol) {
                *buff++ = SYM_SAT_L;
                *buff++ = SYM_SAT_R;
            }
            if (STATE(GPS_FIX_EVER)) {
                OLC_LatLon location;
                // Note: OLC uses both lat and lon. If we are called with a specific value,
                // we'll assume it's part of a Lat/Lon element.
                // However, OSD type 4 usually only prints once.
                location.lat = (double)(coordinateType == GPS_LATITUDE ? gpsValue : gpsSol.llh.lat) / GPS_DEGREES_DIVIDER;
                location.lon = (double)(coordinateType == GPS_LONGITUDE ? gpsValue : gpsSol.llh.lon) / GPS_DEGREES_DIVIDER;
    ...
    case OSD_ELEMENT_TYPE_3: // degree, minutes, seconds style. ddd^mm'ss.00"W
        {
            ...
            tfp_sprintf(buff, "%03u%c%02u%c%02u.%u%c%c", degreesPart, SYM_GPS_DEGREE, minutes, SYM_GPS_MINUTE, seconds, tenthSeconds, SYM_GPS_SECOND, trailingSymbol);
            break;
        }
    ...
    default:
        ...
        if (gpsValue < 0) {
            ...
            *buff++ = ' '; // Add space for sign alignment
        }
        tfp_sprintf(buff, (variantType == OSD_ELEMENT_TYPE_1 ? "%03u.%07u" : "%03u.%04u"), degreesPart, fractionalPart);
        break;
    }
}
#endif // USE_GPS
...
static void osdElementAltitude(osdElementParms_t *element)
{
    ...
    if (haveBaro || haveGps) {
        ...
        if (FLIGHT_MODE(GPS_RESCUE_MODE)) {
            float currentAltMeters = getEstimatedAltitudeCm() / 100.0f;
            float convertedCurrent = osdGetMetersToSelectedUnit(currentAltMeters);
            float targetMeters = gpsRescueGetTargetAltitude() / 100.0f;
            float convertedTarget = osdGetMetersToSelectedUnit(targetMeters);
            unsigned decimalPlaces = (element->type == OSD_ELEMENT_TYPE_2 ? 0 : 1);
            const char unitSymbol = osdGetMetersToSelectedUnitSymbol();

            osdPrintFloat(element->buff, SYM_ALTITUDE, convertedCurrent, "", decimalPlaces, true, SYM_NONE);
            char targetBuff[12];
            osdPrintFloat(targetBuff, '>', convertedTarget, "", decimalPlaces, false, unitSymbol);
            strcat(element->buff, targetBuff);
        } else
#endif
        {
...
static void osdElementReadyMode(osdElementParms_t *element)
{
    ...
    if (phase != RESCUE_IDLE) {
        ...
        case RESCUE_ATTAIN_ALT:
            strcpy(element->buff, "CLIMB");
            break;
        case RESCUE_FLY_HOME:
            strcpy(element->buff, "FLY HOME");
            break;
        case RESCUE_SHUTTLE:
        case RESCUE_SHUTTLE_INFINITE:
            tfp_sprintf(element->buff, "SHUT-%03u", gpsRescueGetCurrentShuttleTrips());
            if (phase == RESCUE_SHUTTLE_INFINITE) {
                element->attr = DISPLAYPORT_SEVERITY_CRITICAL;
            }
            break;
        case RESCUE_SHUTTLE_DESCENT:
            strcpy(element->buff, "S-DESC");
            break;
        case RESCUE_DESCENT:
            strcpy(element->buff, "DESCEND");
            break;
        case RESCUE_LANDING:
            strcpy(element->buff, "LANDING");
            break;
        case RESCUE_ABORT:
            strcpy(element->buff, "ABORT");
            break;
        case RESCUE_COMPLETE:
            strcpy(element->buff, "COMPLETE");
            break;
        case RESCUE_DO_NOTHING:
            strcpy(element->buff, "DO NOTHING");
            element->attr = DISPLAYPORT_SEVERITY_CRITICAL;
            break;
        default:
            break;
        }
    } else if (gpsRescueIsConfigured()) {
        if (!gpsIsHealthy()) {
            strcpy(element->buff, "GPS ERR");
        } else if (!STATE(GPS_FIX) || gpsSol.numSat < gpsRescueConfig()->minSats) {
            tfp_sprintf(element->buff, "SAT %d/%d", gpsSol.numSat, gpsRescueConfig()->minSats);
            element->attr = DISPLAYPORT_SEVERITY_WARNING;
        } else if (!ARMING_FLAG(ARMED)) {
            strcpy(element->buff, "READY");
        } else if (!STATE(GPS_FIX_HOME)) {
            strcpy(element->buff, "NO HOME");
        } else if (!gpsRescueIsAvailable()) {
            strcpy(element->buff, "RES UNV");
        } else {
            strcpy(element->buff, "RES OK");
        }
    }
#endif
}
...
#ifdef USE_GPS_RESCUE
            if (FLIGHT_MODE(GPS_RESCUE_MODE)) {
                direction = (int)(gpsRescueGetTargetDirection() / 10); // cd to dd
            }
#endif
...
#ifdef USE_GPS_RESCUE
        if (FLIGHT_MODE(GPS_RESCUE_MODE)) {
            distance = (int)(gpsRescueGetTargetDistance() / 100); // cm to m
        }
#endif
#ifdef USE_GPS_LAP_TIMER
...
static void osdElementGpsCoordinate(osdElementParms_t *element)
{
    ...
    osdFormatCoordinate(element->buff, gpsValue, coordinateType, element->type, false);

#ifdef USE_GPS_RESCUE
    if (FLIGHT_MODE(GPS_RESCUE_MODE) && element->type != OSD_ELEMENT_TYPE_4) {
        char targetValueStr[20];
        int32_t targetValue = (coordinateType == GPS_LONGITUDE) ? gpsRescueGetTargetLon() : gpsRescueGetTargetLat();
        osdFormatCoordinate(targetValueStr, targetValue, coordinateType, element->type, false);
        // Ensure we don't overflow buff (length 32)
        if (strlen(element->buff) + strlen(targetValueStr) + 2 < OSD_ELEMENT_BUFFER_LENGTH) {
            strcat(element->buff, ">");
            char label[2] = { gpsRescueGetTargetLabel(), '\0' };
            strcat(element->buff, label);
            strcat(element->buff, targetValueStr);
        }
    }
#endif
...
{
    if (STATE(GPS_FIX)) {
        int32_t speed = osdGetSpeedToSelectedUnit(gpsConfig()->gps_use_3d_speed ? gpsSol.speed3d : gpsSol.groundSpeed);
#ifdef USE_GPS_RESCUE
        if (FLIGHT_MODE(GPS_RESCUE_MODE)) {
            int32_t targetSpeed = osdGetSpeedToSelectedUnit((int32_t)gpsRescueGetTargetVelocity());
            tfp_sprintf(element->buff, "%c%3d>%d%c", SYM_SPEED, speed, targetSpeed, osdGetSpeedToSelectedUnitSymbol());
        } else
#endif
        {
            tfp_sprintf(element->buff, "%c%3d%c", SYM_SPEED, speed, osdGetSpeedToSelectedUnitSymbol());
        }
    } else {
...
{
    int throttlePercent = calculateThrottlePercent();
#ifdef USE_GPS_RESCUE
    if (FLIGHT_MODE(GPS_RESCUE_MODE)) {
        throttlePercent = lrintf(gpsRescueGetThrottle() * 100.0f);
    }
#endif
    tfp_sprintf(element->buff, "%c%3d", SYM_THR, throttlePercent);
}
...
{
    // Display servo 0-3 output values (in us) as 4-digit numbers each
    // Format: "1234 1234 1234 1234"
    int s0 = constrain(servo[0], 0, 9999);
    int s1 = constrain(servo[1], 0, 9999);
    int s2 = constrain(servo[2], 0, 9999);
    int s3 = constrain(servo[3], 0, 9999);
    tfp_sprintf(element->buff, "%4d %4d %4d %4d", s0, s1, s2, s3);
}
