...
static void mixerConfigureOutput(void)
{
    ...
    if (isFixedWing()) {
        int activeMotors = 0;
        for (int i = 0; i < mixerRuntime.motorCount; i++) {
            const unsigned reorderedMotorIndex = motorConfig()->dev.motorOutputReordering[i];
            if (motorConfig()->dev.ioTags[reorderedMotorIndex]) {
                activeMotors++;
            } else {
                break;
            }
        }
        mixerRuntime.motorCount = activeMotors;
    }
#ifdef USE_LAUNCH_CONTROL
