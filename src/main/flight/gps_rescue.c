...
// GPS 속도 PI 제어 게인
#define VELOCITY_KP                          3.00f
#define VELOCITY_KI                          0.70f
...
static float calculateVelocityThrottle(void)
{
    ...
    float hoverMax = fminf(throttleHover + 300.0f, throttleMax - 100.0f);
...
static void handleShuttlePhase(void)
{
    ...
    if (rescueState.phase != RESCUE_SHUTTLE && rescueState.phase != RESCUE_SHUTTLE_INFINITE) return;
...
{
    handleShuttleProgress();
    if (rescueState.phase != RESCUE_SHUTTLE_DESCENT) return;
...
static void sensorUpdate(void)
{
    ...
    rescueState.sensor.groundSpeedCmS = (uint16_t)fmaxf(0.0f, pt3FilterApply(&velocityUpsampleLpf, (float)gpsSol.groundSpeed));
...
static bool checkGPSRescueIsAvailable(void)
{
    ...
    if (!gpsIsHealthy() || (!STATE(GPS_FIX_HOME) && !STATE(GPS_FIX))) return false;
...
bool gpsRescueIsDisabled(void) { return (!STATE(GPS_FIX_HOME) && !STATE(GPS_FIX)); }
