...
static float       altbankgain     =   0.0f; // 뱅크턴 중 고도 보정 게인 (Profile3 Pitch d_min) 0=비활성
static float       altBankPitchCorrFactor = 1.0f; // altbankgain 피치 보정 계수 (handleShuttleProgress→calculateAltitudePitch 전달)
...
static float calculateVelocityThrottle(void)
{
    ...
    // 쓰로틀은 목표 속도 유지만 담당. 하강률은 calculateAltitudePitch(피치+altbankgain)가 담당.
...
static float calculateAltitudePitch(float altErrM, bool isShuttlePhase, bool descentAllowed)
{
    ...
    // altbankgain 활성 시 셔틀 구간의 sbankPitchGain을 고도 오차 방향으로 보정
    // altBankPitchCorrFactor는 handleShuttleProgress에서 계산되어 전달됨 (셔틀 외 구간은 항상 1.0)
    float currentBankPitchGain = isShuttlePhase ? sbankPitchGain * altBankPitchCorrFactor : bankPitchGain;
...
static void handleShuttleProgress(void)
{
    ...
    // altbankgain 뱅크턴 고도 보정 (altbankgain > 0 && 셔틀에만 적용)
    // 근접도(포인트 접근) × 뱅크각 두 조건이 동시에 성립할 때만 보정 활성화
    // - 직선 왕복 패턴: 뱅크 작음 → rollFade ≈ 0 → 보정 없음
    // - 포인트에서 먼 중간 직선 구간: proximityFade ≈ 0 → 보정 없음
    // - 포인트 근처 뱅크턴 중: 두 fade 모두 1 → 보정 최대
    altBankPitchCorrFactor = 1.0f; // 기본값 초기화 (calculateAltitudePitch에서 사용)
    if (altbankgain > 0.01f) {
        ...
        // 근접도: activationThreshold 이내로 들어올수록 1.0 (포인트에서 멀면 0)
        float proximityFade = constrainf(1.0f - (dCm / activationThresholdCm), 0.0f, 1.0f);
        // 뱅크각: 35°+ 에서 1.0, 15° 이하에서 0 (직선 구간에서 보정 소멸)
        float rollFade = constrainf((currentRollDeg - 15.0f) / 20.0f, 0.0f, 1.0f);
        float fade = proximityFade * rollFade;

        // 뱅크각 보정: 고도 높으면 뱅크 증가(하강 유도), 낮으면 뱅크 감소(상승 유도)
        float bankCorrection = constrainf(1.0f + altbankgain * altErrM * fade, 0.6f, 1.4f);
        targetBankDeg *= bankCorrection;

        // 피치 보정 계수: 뱅크와 반대 방향 (calculateAltitudePitch에서 sbankPitchGain에 적용)
        // 고도 높으면 뱅크피치 감소(기수 덜 들림), 낮으면 증가(기수 더 들림)
        altBankPitchCorrFactor = constrainf(1.0f - altbankgain * altErrM * fade, 0.6f, 1.4f);
    }
...
static void handleDescentPhase(void)
{
    ...
    float endVel = landingAlt * 100.0f;
...
static void updateRescueParams(void)
{
    ...
    altbankgain     = constrainf((float)profile3->d_min[PID_PITCH] / 100.0f, 0.0f, 0.5f);
